...
def teacher_info():
    ...
    #status=0表示未处理的信息
    modify_info = list(agent_feedback.find({'status':0}))
...
def data_preservation():
    ...
    print("_id",request.form.get("object_id"))
    ...
    #处理新增数据
        ...
        #将商务反馈的记录标注为已处理
        agent_feedback_collection.update_one({'_id':obj_id},{'$set':{'status':1}})
        print(request.form.get('institution'))
    else:
    #处理修改数据
        ...
        # 将商务反馈的记录标注为已处理
        print(request.form.get('edu_exp'))
        agent_feedback_collection.update_one({'_id': obj_id}, {'$set': {'status': 1}})
